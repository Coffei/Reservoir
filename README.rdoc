...
1. Setup your database:
       ...
       1.3. Configure your DB in reservoir by editing config/database.yml file. 
              (See rails tutorials for further instruction). By default it is using postgres
              database named "reservoir" on localhost with username and password "reservoir".
2. Setup Reservoir:
       2.1. Run "bundle install" in the Reservoir dir.
       2.2. Run "bundle exec rake db:migrate" to create tables with appropriate schema.
       2.3. Run "bundle exec rake assets:precompile" to compile assets
            Note: This command might take a while (you can run it with --trace to see the progress)
       2.4 Package Reservoir to war by command "warble war".
3. Deploy:
